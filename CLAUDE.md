...
2. **Never put a raw colour literal in a semantic token** (code or Figma). Literals live only in the primitive layer (`--neutral-*`/`--lia-*`, `colors/*`); semantics alias primitives. See Aliasing Hierarchy.
